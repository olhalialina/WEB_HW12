...
async def get_contacts_first_name(first_name: str, user: User, db: Session) -> List[Contact]:
    return db.query(Contact).filter(and_(Contact.first_name.like(f'%{first_name}%'), Contact.user_id == user.id)).all()


async def get_contacts_last_name(last_name: str, user: User, db: Session) -> List[Contact]:
